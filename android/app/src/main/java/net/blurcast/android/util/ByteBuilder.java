...
public class ByteBuilder {
    ...
    public byte[] getBytes(int maxLength) {
        if(bytes.length > maxLength) {
            byte[] subBytes = new byte[maxLength];
            System.arraycopy(bytes, 0, subBytes, 0, maxLength);
            return subBytes;
        }
        else {
            return bytes;
